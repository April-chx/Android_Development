...
<html>
	...
	<script type="text/javascript">
		...
		function arrangement_tomorrow1()
		{     
            var showTomorrow = document.arrangement_tomorrow.showTomorrow.value;
            var table = document.getElementById("table");
            var info = showTomorrow.split("|");                     
            ...
            loadXMLDoc("./userSql.php?movie="+info[1]+"&arrangement_tomorrow="+1,function()
            {	
        ...
        function arrangement_tomorrow2(num)
        {
            selectIndex = document.getElementById("showTomorrow").selectedIndex;  
            document.cookie = 'selectIndex =' + selectIndex;

            loadXMLDoc("./userSql.php?num="+num+"&arrangement_tomorrow="+2,function()
            {
                if (xmlhttp.readyState==4 && xmlhttp.status==200)
                {
                    window.location.assign("arrangement_tomorrow.php");
                }
            });	
        }
        
        function arrangement_tomorrow3()
        {
            var start_time = document.arrangement_tomorrow.start_time.value;
            var show_type = document.arrangement_tomorrow.show_type.value;
            var ticket_price = document.arrangement_tomorrow.ticket_price.value;
            var showTomorrow = document.arrangement_tomorrow.showTomorrow.value;
            var info = showTomorrow.split("|");  
            ...
            loadXMLDoc("./userSql.php?movie="+info[1]+"&start_time="+start_time+"&show_type="+show_type+"&ticket_price="+ticket_price+"&arrangement_tomorrow="+3,function()
            {
                if (xmlhttp.readyState==4 && xmlhttp.status==200)
                {
                    window.location.assign("arrangement_tomorrow.php");
        ...
        function arrangement_tomorrow4()
        {
            var new_movie = document.arrangement_tomorrow.new_movie.value;
            var new_start_time = document.arrangement_tomorrow.new_start_time.value;
            var new_show_type = document.arrangement_tomorrow.new_show_type.value;
            var new_ticket_price = document.arrangement_tomorrow.new_ticket_price.value;
            
            selectIndex = document.getElementById("showTomorrow").selectedIndex;  
            document.cookie = 'selectIndex =' + selectIndex;
            
            loadXMLDoc("./userSql.php?new_movie="+new_movie+"&new_start_time="+new_start_time+"&new_show_type="+new_show_type+"&new_ticket_price="+new_ticket_price+"&arrangement_tomorrow="+4,function()
            {
                if (xmlhttp.readyState==4 && xmlhttp.status==200)
                {
                    window.location.assign("arrangement_tomorrow.php");
        ...
        function arrangement_tomorrow5()
        {
            var showTomorrow = document.arrangement_tomorrow.showTomorrow.value;
            var info = showTomorrow.split("|");   
            
            loadXMLDoc("./userSql.php?movie="+info[1]+"&arrangement_tomorrow="+5,function()
            {
                if (xmlhttp.readyState==4 && xmlhttp.status==200)
                {
                    window.location.assign("arrangement_tomorrow.php");
        ...
        {  
        	...
        	{  
        		...
        		document.getElementById("showTomorrow").options[json.selectIndex].selected = true;  
...
<div class="container">
    ...
    <div class="content">        
        <form name="arrangement_tomorrow" action="arrangement_tomorrow.php" method="post">
        <?php
			error_reporting(0);
			session_start();
			...
			echo "<br><br><h1 class='m_2'>>>&nbsp排片管理</h1>
            		<a href='arrangement.php' style='font-family:微软雅黑;font-size:16px;margin-left:70px'>今日排片</a>
                    <font style='font-size:17px'>&nbsp&nbsp|&nbsp&nbsp</font>
                    <font style='font-family:微软雅黑;font-size:16px;'>明日排片</font>";	
			echo "<font style='margin-left:100px'>添加新片：</font>
                    ...
                    <a href='javascript:;' onclick='arrangement_tomorrow4()' style='margin-left:5px;font-family:微软雅黑' >[添加]</a><br><br><br>";
			?>
		...
		<?php
			...
			else
			{
                ...
                $res_temp=mysql_query("SELECT DISTINCT 片名 FROM movie_arrangement_tomorrow WHERE 影院='$user_name'",$con);
                ...
                <select name='showTomorrow' id='showTomorrow' onchange='arrangement_tomorrow1()'>";
                
                $res=mysql_query("SELECT DISTINCT 片名 FROM movie_arrangement_tomorrow WHERE 影院='$user_name'",$con);
                ...
                echo "</select><a href='javascript:;' onclick='arrangement_tomorrow5()' style='margin-left:10px;font-family:微软雅黑' >[删除该片]</a><br><br>";
                
                echo "<div class='register-top-grid'><div><img id='poster' style='width:150px;margin-left:265px' src='submissions/$row_poster[0]'/></div>";
                //echo "<span id='poster1'>ppp</spam>";

				$sql_first=mysql_query("SELECT time_format(时间, '%H:%i'),播放类型,票价,编号 FROM movie_arrangement_tomorrow 
                ...
                while($row_info_first=mysql_fetch_array($sql_first))
            	{
                    echo "<tr><td style='text-align:center;'>$row_info_first[0]</td>
                        ...
                        <td style='text-align:center;'><a href='javascript:;' onclick='arrangement_tomorrow2($row_info_first[3])' style='font-family:微软雅黑' >[删除]</a></td>";
            	}
                echo "</tr></table><br>";
                ...
                <a href='javascript:;' onclick='arrangement_tomorrow3()' style='margin-left:5px;font-family:微软雅黑' >[添加]</a></div><div class='clearfix'> </div></div>";
            }
			?>
